...
|					.xwm - music										|
|																		|
|	© 2013 Full Sail, Inc. All rights reserved. The terms "Full Sail", 	|
...
namespace SGD
{
	namespace SGD_IMPLEMENTATION
	{
		...
		public:
			...
			virtual int			GetMasterVolume		( VolumeType type )					override;
			virtual bool		SetMasterVolume		( VolumeType type, int value )		override;

			//virtual int 		GetMusicMasterVolume		( void )					override;
			//virtual bool		SetMusicMasterVolume		( int value )				override;
			//virtual int 		GetSoundEffectsMasterVolume	( void )					override;
			//virtual bool		SetSoundEffectsMasterVolume	( int value )				override;
	...
	namespace SGD_IMPLEMENTATION
	{
		...
		bool AudioManager::Initialize( void )
		{
			...
			hResult = XAudio2Create( &m_pXAudio, XAUDIO2_DEBUG_ENGINE );
#else
		...
		//*************************************************************//
		// GET MASTER VOLUME
		int AudioManager::GetMasterVolume( VolumeType type )
		{
			// Sanity-check the wrapper's status
			...
			assert( (type == MASTER_MUSIC || type == MASTER_SOUND_EFFECTS) && "AudioManager::GetMasterVolume - invalid type" );


			// Get the master volume from the submix voice
			float fVolume = 0.0f;
			if( type == MASTER_MUSIC )
				m_pMusVoice->GetVolume( &fVolume );
			else //if( type == MASTER_SOUND_EFFECTS )
		...
		//*************************************************************//
		// SET MASTER VOLUME
		bool AudioManager::SetMasterVolume( VolumeType type, int value )
		{
			// Sanity-check the wrapper's status
			...
			// Sanity-check the parameter
			assert( (type == MASTER_MUSIC || type == MASTER_SOUND_EFFECTS) && "AudioManager::SetMasterVolume - invalid type" );
