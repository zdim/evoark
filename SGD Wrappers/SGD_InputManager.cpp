...
namespace SGD
{
	...
	namespace SGD_IMPLEMENTATION
	{
		...
		bool InputManager::Initialize( void )
		{
			...
			std::vector< GUID > ids;
			hResult = m_pDirectInput->EnumDevices( DI8DEVCLASS_GAMECTRL, &EnumerateJoystick, &ids, DIEDFL_ATTACHEDONLY );
			if( FAILED( hResult ) )
			{
				// MESSAGE
				char szBuffer[ 128 ];
				_snprintf_s( szBuffer, 128, _TRUNCATE, "!!! InputManager::Update - failed to enumerate DirectInput devices (0x%X) !!!\n", hResult );
				OutputDebugStringA( szBuffer );
			}
			...
			if( m_vGamepads.size() != ids.size() )
			{
				// MESSAGE
				char szBuffer[ 128 ];
				_snprintf_s( szBuffer, 128, _TRUNCATE, "!!! InputManager::Initialize - failed to initialize a DirectInput device !!!\n" );
				OutputDebugStringA( szBuffer );
			}
...
#undef UPDATE_KEY
			...
			for( unsigned int i = 0; i < m_vGamepads.size(); i++ )
			{
				...
				if( FAILED( hResult ) )
				{
					...
					if( FAILED( hResult ) )
					{
						info = GamepadInfo{ info.id, info.pDevice, info.bHasTriggerAxis, false };
						continue;
					}
				}
				...
				// Interpret the point-of-view hat / dpad
		...
		/*static*/ BOOL CALLBACK InputManager::EnumerateJoystick( const DIDEVICEINSTANCEW* instance, void* extra )
		{
			...
			std::vector< GUID >* ids = reinterpret_cast< std::vector< GUID >* >( extra );

			// Store the joystick id
			ids->push_back( instance->guidInstance );
